...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Volumes/GoogleDrive/Team Drives/EEG Research/BluegrassWMPlatform_PsychopyV2/BluegrassWM.py',
...
List1 = inbook.sheet_by_index(0)
List2 = inbook.sheet_by_index(1)
List3 = inbook.sheet_by_index(2)
List4 = inbook.sheet_by_index(3)
List5 = inbook.sheet_by_index(4)
List6 = inbook.sheet_by_index(5)
List7 = inbook.sheet_by_index(6)
List8 = inbook.sheet_by_index(7)
...
#Target1=[]  #first stimulus in AX-AY-BX-BY
#Jitter1=[]
#read the stimuli from our sheet
#for rowx in range(num_rows):
#    row = List1.row_values(rowx)
#    Jitter1.append(row[3])


#row = List1.row_values(0)
#Target1.append(row[9])
...
whiteWaitInterTrial = visual.ImageStim(
    ...
    ori=0, pos=(0, 0), size=pictureSize,
    color=[1,1,1], colorSpace='rgb', opacity=1,
    flipHoriz=False, flipVert=False,
...
TrialListIteration = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('lib\\list3.xlsx'),
...
for thisTrialListIteration in TrialListIteration:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    # Find images of procedure using 'Iteration' column
    procLine, aa = Iteration.split(":")
    #print(proc)
    procIndex = int(procLine)
    
    
    
    TrialDuration=Duration/1000+Jitter/1000
    ITDuration=ISI/1000+Jitter/1000
    greenProc.setSize(greenSize)
    ProcTarget.setSize(pictureSize)
    ProcTarget.setImage(Target2)
    ProcPicture.setSize(pictureSize)
    ProcPicture.setImage(Picture2)
    # Send trial onset marker to LSL
    ...
    while continueRoutine:
        ...
        frameRemains = 0.0 + ITDuration- win.monitorFramePeriod * 0.75  # most of one frame period left
    ...
    trials = data.TrialHandler(nReps=1, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('lib\\list3.xlsx', selection=Iteration),
    ...
    for thisTrial in trials:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        TrialPicture.setImage(Picture)
        keyPressTrial = event.BuilderKeyResponse()
        #ITDuration=ISI/1000+Jitter/1000
        # Send trial onset marker to LSL
        #outlet.push_sample(int(correctseth))
        # Send trial onset marker to LSL
        ...
        while continueRoutine:
            ...
            frameRemains = 0.0 + Duration/1000- win.monitorFramePeriod * 0.75  # most of one frame period left
            ...
            if keyPressTrial.status == STARTED:
                ...
                if "escape" in theseKeys:
                    endExpNow = True
                if len(theseKeys) > 0:  # at least one key was pressed
                    keyPressTrial.keys.extend(theseKeys)  # storing all keys
                    keyPressTrial.rt.append(keyPressTrial.clock.getTime())
            # Send key press marker to LSL
            #outlet.push_sample(int(keyPressTrial.keys))
        ...
        # Send key press marker to LSL
        #outlet.push_sample(int(keyPressTrial.keys)[0])
...
# completed 1 repeats of 'TrialListIteration'
