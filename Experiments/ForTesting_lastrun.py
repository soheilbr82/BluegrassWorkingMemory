...
if len(sys.argv) > 1:
    ...
    expInfo = {'Participant ID': '', 'Age': '', 'Sex': ['Male', 'Female'],
                        ...
                        'Experiment Mode': ['intro', 'Resting_Eyes_Opened', 'Resting_Eyes_Closed',
                                            'train', 'neurofeedback'], 'Visualize Epochs': False, 'Audiovisual Cue': [False],
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='D:\\Projects\\BluegrassSTM_1.7_WithAudiovisual_Win64\\Experiments\\ForTesting_lastrun.py',
...
green_fb_1 = visual.ShapeStim(
    win=win, name='green_fb_1', vertices='star7',
    size=[1.0, 1.0],
    ori=0, pos=(0, 0),
    lineWidth=1, lineColor='green', lineColorSpace='rgb',
    fillColor='green', fillColorSpace='rgb',
...
green_fb_2 = visual.ShapeStim(
    win=win, name='green_fb_2', vertices='star7',
    size=[1.0, 1.0],
    ori=0, pos=(0, 0),
    lineWidth=1, lineColor='green', lineColorSpace='rgb',
    fillColor='green', fillColorSpace='rgb',
...
while continueRoutine:
    ...
    if keyPressRun_1.status == STARTED:
        ...
        if len(theseKeys):
            ...
            if "escape" == theseKeys:
                endExpNow = True
            keyPressRun_1.keys = theseKeys.name  # just the last key pressed
            keyPressRun_1.rt = theseKeys.rt
...
for thisA1_Block in A1_Blocks:
    ...
    for thisA1_trial in A1_trials:
        ...
        while continueRoutine:
            ...
            if keyPressTrial_1.status == STARTED:
                ...
                if len(theseKeys):
                    ...
                    if "escape" == theseKeys:
                        endExpNow = True
                    if keyPressTrial_1.keys == []:  # then this was the first keypress
                        keyPressTrial_1.keys = theseKeys.name  # just the first key pressed
                        keyPressTrial_1.rt = theseKeys.rt
            # Send trial onset marker to LSL
            ...
            if green_1.status == STARTED:  # only update if drawing
                green_1.setOpacity(greenDotOpac, log=False)
            ...
            if red_1.status == STARTED:  # only update if drawing
                red_1.setOpacity(redDotOpac, log=False)
        ...
        while continueRoutine:
            ...
            feedbackDelay = 0
            #if len(streams)>0:  
            #    feedback, timestamps = inlet.pull_chunk()
            #if len(feedback) > 0:
            #    print(feedback)
            #    feedbackLabel = feedback[0][0]
            #    feedbackDelay = feedback[0][1]
                
                # Label == 1 means fast and Label == 0. means slow
            #    if feedbackLabel == 1.0:
            #        tone.correct()
            #        greenDotOpac = 1
            #        greenFBSize = feedbackDelay*0.3
                    #print('correct')
            #    elif feedbackLabel == 0.0:
            #        tone.incorrect()
            #        redDotOpac = 1
            #        redFBSize = feedbackDelay*0.3
                        #print('incorrect')
        ...
        # keep track of which components have finished
        InterTrialComponents = [blackWaitInterTrial_2, whiteWaitInterTrial_2, green_fb_1, red_fb_1]
        ...
        while continueRoutine:
            ...
            if green_fb_1.status == STARTED:  # only update if drawing
                green_fb_1.setOpacity(greenDotOpac, log=False)
                green_fb_1.setSize((greenFBSize, greenFBSize), log=False)
            ...
            if red_fb_1.status == STARTED:  # only update if drawing
                red_fb_1.setOpacity(redDotOpac, log=False)
                red_fb_1.setSize((redFBSize, redFBSize), log=False)
    ...
    for thisA2_trial in A2_trials:
        ...
        while continueRoutine:
            ...
            if green_2.status == STARTED:  # only update if drawing
                green_2.setOpacity(greenDotOpac, log=False)
            ...
            if red_2.status == STARTED:  # only update if drawing
                red_2.setOpacity(redDotOpac, log=False)
        ...
        # keep track of which components have finished
        InterTrial_2Components = [blackWaitInterTrial_4, whiteWaitInterTrial_4, green_fb_2, red_fb_2]
        ...
        while continueRoutine:
            ...
            if green_fb_2.status == STARTED:  # only update if drawing
                green_fb_2.setOpacity(greenDotOpac, log=False)
                green_fb_2.setSize((greenFBSize, greenFBSize), log=False)
            ...
            if red_fb_2.status == STARTED:  # only update if drawing
                red_fb_2.setOpacity(redDotOpac, log=False)
                red_fb_2.setSize((redFBSize, redFBSize), log=False)
