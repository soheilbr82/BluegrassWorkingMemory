...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Projects\\BluegrassSTM_1.8_WithoutAudiovisual_Win64\\Experiments\\Resting_Eyes_Closed_lastrun.py',
...
# keep track of which components have finished
desEyesCloseComponents = [polygon, descriptionText, keyPressRun]
...
t = 0
_timeToFirstFrame = win.getFutureFlipTime(clock="now")
desEyesCloseClock.reset(-_timeToFirstFrame)  # t0 is time of first possible flip
frameN = -1

# -------Run Routine "desEyesClose"-------
while continueRoutine:
    # get current time
    t = desEyesCloseClock.getTime()
    tThisFlip = win.getFutureFlipTime(clock=desEyesCloseClock)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
